...
def product(request):
    ...
    It used to render product.html, a template that does not exist in this
    project, so the URL answered every request with a 500. Nothing links to it,
    but it is a public address that may sit in old bookmarks or search results,
    so it forwards to the catalogue instead of being deleted outright.
    """
...
def shop_details(request):
    """
    Legacy /shop-details/ URL - shop-details.html was never added, so this
    answered with a 500. Forwarded to the catalogue for the same reason as
    product() above.
    """
    return redirect('shop')
