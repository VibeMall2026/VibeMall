...
# ==================== EMAIL LOG & NOTIFICATION MODELS ====================

class EmailLog(models.Model):
    """Track all outgoing emails for debugging and audit"""
    EMAIL_TYPE_CHOICES = [
        ('ORDER_CONFIRMATION', 'Order Confirmation'),
        ('ORDER_STATUS_UPDATE', 'Order Status Update'),
    ...
    def __str__(self):
        status = 'Sent' if self.sent_successfully else 'Failed'
        return f"[{status}] {self.email_type} → {self.email_to} ({self.created_at.strftime('%Y-%m-%d %H:%M')})"
