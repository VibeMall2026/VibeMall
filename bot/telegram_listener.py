...
def _build_help_text() -> str:
    return (
        ...
        "/signal_forge_stop [time] - Stop the Signal Forge account until next XAUUSD reopen or a custom time\n"
        "/signal_forge_start - Start the Signal Forge account now\n"
...
def _build_signal_forge_overview_text(acc) -> str:
    ...
    if not acc or not acc.enabled:
        return "Signal Forge account overview unavailable: target account not available."

    if not _connect_account(acc):
        return (
            "Signal Forge account overview unavailable: could not connect to the account.\n"
            f"Account: {acc.label}\n"
            f"Login: {acc.login}"
        )
