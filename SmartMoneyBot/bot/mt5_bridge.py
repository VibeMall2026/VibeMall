...
def _launch_mt5_terminal(path: str) -> bool:
    ...
    try:
        ...
        logger.info(f"MT5 terminal launched: {terminal_path}{' /portable' if getattr(config, 'MT5_PORTABLE', False) else ''}")
        return True
    ...
    with get_mt5_lock():
        if not mt5.initialize(**kwargs):
            ...
            if "IPC send failed" in last_error or "IPC timeout" in last_error:
                ...
                if launch_path and _is_terminal_running(launch_path):
                    # Terminal process is alive but hasn't finished its own startup
                    # handshake yet (e.g. first login / server discovery on a real
                    # account can take longer than one IPC timeout). Killing and
                    # relaunching here would restart that handshake from scratch on
                    # every retry and never let it finish — just wait instead.
                    logger.info(
                        "MT5 terminal is still starting up; waiting instead of relaunching"
                    )
                    time.sleep(10)
                    if mt5.initialize(**kwargs) and mt5.login(config.MT5_LOGIN, password=config.MT5_PASSWORD, server=config.MT5_SERVER):
                        ...
                        if info and int(getattr(info, "login", 0) or 0) == int(config.MT5_LOGIN):
                            ...
                            logger.info(
                                f"MT5 reconnected after waiting for terminal startup | Account: {info.login} | "
